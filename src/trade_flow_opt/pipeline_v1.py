...
EXTERNAL_ROOT = Path(r"E:\zjl\CMU\research\website")
BASELINE_REPO_ROOT = EXTERNAL_ROOT / "EV-Battery-Chemistry-Trade-Flow-Analysis_2.1"
DATA_ROOT = EXTERNAL_ROOT / "EV-Battery-Chemistry-Trade-Flow-Analysis_2.0" / "data"
...
def _ensure_external_repo_on_path() -> None:
    repo = str(BASELINE_REPO_ROOT)
    if repo not in sys.path:
        # Keep the current trade_flow_opt workspace ahead of the frozen 2.1 repo so
        # local battery_7step_site fixes feed both output generation and the viewer.
...
def _override_original_inputs(metal: str, year: int, inputs):
    if metal != "Ni":
        return inputs
    # Original baseline in trade_flow_opt should reflect all Ni 2nd-post-trade Comtrade folders,
...
def transition_contexts(metal: str, inputs, *, cobalt_mode: str = DEFAULT_COBALT_MODE) -> dict[str, TransitionContext]:
    ...
    if metal == "Li":
        contexts["trade1"] = TransitionContext(
            ...
            _sum_maps(inputs.processing_brine_balance, inputs.processing_lithium_ores_balance),
        ...
        contexts["trade2"] = TransitionContext(
            ...
            _sum_maps(inputs.refining_hydroxide_balance, inputs.refining_carbonate_balance),
